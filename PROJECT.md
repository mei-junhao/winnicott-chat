...
### 6.5 自适应回应方式模块新增说明（2026-08-04）

1. **为什么现有入口无法扩展**：现有“温度”只由用户在设置中手动调节，且对普通用户隐藏较深；把语境识别、迟滞、确认条、会话状态和来访者友好回复规则分别复制到温尼科特页与多大师页，会形成两套容易漂移的判断逻辑。`safety.js` 只负责危机阻断，不能承担非危机语境分类；`prompt-contracts.js` 只提供静态事实与术语契约，也不应管理运行时交互状态。
2. **修改成本**：新增一个无依赖静态模块 `public/adaptive-dialogue.js`，集中判断“当前更需要倾听还是理论分析”，输出本轮回复契约，并渲染可撤销的轻量确认条；接入 `winnicott-chat.html` 与 `master-chat.html`，不接入锁定为专业用途的 `consultant-a.html`、督导页或圆桌。模块不新增模型调用。
3. **未来维护方式**：只保存本次会话的回应偏好，不保存“来访者身份”；识别词表、阈值、迟滞轮数和微文案统一在模块内维护。新增单聊入口时通过 `AdaptiveDialogue.configure()` 配置独立 `scope` 与温度更新回调，并使用 `inspect()`、`instruction()`、`showSuggestion()` 接入发送流程。
4. **职责重叠检查**：模块负责非危机语境识别、支持/平衡/分析三种回应方式、一次性确认和会话内偏好；危机仍由 `safety.js` 优先处理，事实/术语仍由 `prompt-contracts.js` 约束，知识文件加载与 API 请求仍由页面原逻辑负责。

### 6.6 部署命令（GitHub Pages）
...
### 6.8 关键代码位置（winnicott-chat.html）
...
- 远端 `/opt/chat-proxy/server.js`：旧版 DeepSeek 透传兜底路径对 `deepseek-v4-*` 增加 `enable_thinking:false`，彻底关闭思考 token，避免推理占用回复预算。
...
| 日期 | 变更内容 |
|------|---------|
| 2026-08-04 | **v5.5.2 修复大师音色角色匹配**：统一温尼科特/拉康、通用大师、咨询师A、AI督导的 TTS role；圆桌按实际发言大师绑定音色；VPS `/tts` 增加 `susan_johnson` 规范别名；代表性角色音频请求返回 `200 audio/mpeg` |
| 2026-08-05 | **v5.5.1 修复流式语音朗读按钮**：五个 TTS 页面增强气泡文本监听，按钮被流式 `innerHTML` 替换后自动补回并同步最新回复文本；温尼科特、通用大师、咨询师页在流式回复完成时显式收口；线上 `/tts` 返回 `200 audio/mpeg` 回归通过 |
| 2026-08-04 | **v5.5.0 方案 D 自适应回应方式（已部署）**：普通单聊页根据当前消息识别支持、平衡或分析需要；高置信度倾诉先以短句、少术语、每轮最多一个问题回应，再显示一次性轻量确认；明确知识请求直接进入专业模式；危机分流始终优先；用户手动温度优先；仅在 `sessionStorage` 保存当前会话状态，不保存“来访者身份” |
