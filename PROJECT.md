...
| 日期 | 变更内容 |
|------|---------|
| 2026-08-04 | **v5.5.0 方案 D 自适应回应方式（本地完成，未部署）**：普通单聊页根据当前消息识别支持、平衡或分析需要；高置信度倾诉先以短句、少术语、每轮最多一个问题回应，再显示一次性轻量确认；明确知识请求直接进入专业模式；危机分流始终优先；用户手动温度优先；仅在 `sessionStorage` 保存当前会话状态，不保存“来访者身份” |
